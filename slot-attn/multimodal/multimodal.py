...
class VisionEncoder(nn.Module):
    ...
    def _load_pretrained_cnn(self):
        ...
        if self.cnn_dino:
            ...
            model_name = self.cnn_model  # models/TC-S-resnext.tar
            ...
            if not hasattr(torchvision.models, model_name):
                ...
                for name, model_name_ in name_to_model_name.items():
                    if name in model_name:
                        model_name = model_name_    #'resnext50_32x4d'
                        break
                else:
                    assert False, \
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, vocab, image_feature_map_dim, args):
        ...
        self.text_encoder = self.args.get("text_encoder")    #embedding
        ...
        # build model
        if self.text_encoder in ["lstm", "bilstm"]:  #no
