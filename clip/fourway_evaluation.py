...
parser.add_argument('--json_path',type=str,default="/scratch/wz3008/SlotAttn/clip/config/eval_dev.json")
...
parser.add_argument('--arch', default='slotcontrast', type=str, choices=['slotcontrast','steve'])
...
def main():
    ...
    elif args.arch == 'steve':
        vision_encoder = STEVE_Wrapper(model_config.steve)
        vision_encoder.feat_fc = torch.nn.Linear(in_features=192, out_features=512, bias=True)
        vision_encoder.slot_fc = torch.nn.Linear(in_features=192, out_features=512, bias=True)
    
    model = Multimodalmodel(vision_encoder, text_encoder)
    print(f"load {args.arch}")
