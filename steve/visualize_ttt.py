...
def visualize(video, recon_dvae, recon_tf, attns, N=1):
    ...
    frames = []
    attns = add_border_to_attn_images(attns, color=(0., 0., 0.), thickness_px=1, thr=0.2, use_contour=True, use_bbox=False)  # (N, T, num_slots, 3, 128, 128)
    for t in range(T):
...
for path in file_paths:
    ...
    model.train()
    # optimizer = torch.optim.Adam([p for p in model.parameters()], lr=1e-5, weight_decay=0.1)
    optimizer = torch.optim.Adam([
        {'params': (x[1] for x in model.named_parameters() if 'dvae' in x[0]), 'lr': 3e-4},
        {'params': (x[1] for x in model.named_parameters() if 'steve_encoder' in x[0]), 'lr': 1e-4},
        {'params': (x[1] for x in model.named_parameters() if 'steve_decoder' in x[0]), 'lr': 3e-4},
    ])
...
vutils.save_image(frames, f"/home/yy2694/scratch/SlotAttn/steve/visualize_frames/TTT/{Path(args.checkpoint_path).stem}.png")
print(f"saved /home/yy2694/scratch/SlotAttn/steve/visualize_frames/TTT/{Path(args.checkpoint_path).stem}.png")
