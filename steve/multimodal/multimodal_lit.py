...
from multimodal.multimodal import VisionEncoder
...
def foo():
    ...
    print(f"current at the function:      [{caller.f_code.co_name}]")
...
class MultiModalLitModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if not self.lr_scheduler: #self.lr_scheduler=False
            return optimizer
    ...
    def calculate_joint_loss(self, batch, stage, log, eval_textgen=False,
        ...
        x, y, y_len, raw_y = batch  # (img, utterance_idxs, utterance_length, raw_utterances)
        ...
        if self.lambda_mm or not self.optimize_unused: # do
        ...
        if self.lambda_lm or not self.optimize_unused: # do
            # calculate language model ce loss
            ...
            if self.language_model.text_encoder.has_attention: # not do
    ...
    def validation_test_step(self, stage, batch, batch_idx, dataloader_idx=0):
        ...
        # batch: (
        #     img: [16,3,224,224], 
        #     utterance_idxs: [16,17],
        #     utterance_length: [16], 
        #     raw_utterances: list(16): utterance,
        # )
