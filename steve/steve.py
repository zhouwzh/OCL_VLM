...
class ResNextWrapper(nn.Module):
    def __init__(self, args):
        ...
        if args.dataset == "movie":
            ...
            # print(f"\nUsing eminorhan pretrained model dino_s_resnext50\n")
            # import time; time.sleep(3)
            cvcl, preprocess = MultiModalLitModel.load_model(model_name="cvcl")
            model = cvcl.vision_encoder.model
            print(f"\nUsing resnext50 from cvcl\n")
            import time; time.sleep(3)
...
class STEVE(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.use_backbone:
            ...
            if not args.use_dvae:
                self.mlp_backbone = nn.Conv2d(256, args.vocab_size, kernel_size=1)
    ...
    def forward(self, video, tau, hard):
        ...
        if self.use_backbone:
            backbone_feature = self.backbone(video_flat)   # B*T,256,32,32
            # 72, 512, 16, 16
            # 72, 1024, 8, 8
            # 72, 2048, 4, 4
        ...
        if self.use_dvae:
            ...
            z_logits = F.log_softmax(self.mlp_backbone(backbone_feature), dim=1)
    ...
    def encode(self, video):
        ...
        # emb = self.steve_encoder.cnn(backbone_feature)
        emb = self.steve_encoder.pos(emb)             # B * T, cnn_hidden_size, H, W
        H_enc, W_enc = emb.shape[-2:]
    ...
    def visualize_recon(self, video, tau, hard):
        B, T, C, H, W = video.size()
        ...
        best_slot_id = attns.argmax(dim=2)  # B, T, 1, H, W
        slot_mask = (best_slot_id.unsqueeze(2) == torch.arange(self.num_slots).to(video.device).view(1, 1, self.num_slots, 1, 1, 1))  # B, T, num_slots, 1,H, W
        slot_mask = slot_mask.float()
        # slot_mask = torch.nn.functional.one_hot(best_slot_id, num_classes=self.num_slots).permute(0,1,5,2,3,4).float()  # B, T, num_slots, 1,H, W
        attns = video.unsqueeze(2) * slot_mask + (1. - slot_mask)  # B, T, num_slots, C, H, W
